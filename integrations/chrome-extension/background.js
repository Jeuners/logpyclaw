/**
 * LogpyClaw Browser Skill — Service Worker (background.js)
 *
 * Verbindet sich per WebSocket mit LogpyClaw (ws://localhost:6060/api/chrome/ws).
 * Empfängt Befehle, führt sie aus, sendet Ergebnisse zurück.
 */

const WS_URL = 'ws://localhost:6060/api/chrome/ws';
...
function startKeepalive() {
  ...
  keepaliveTimer = setInterval(() => {
    if (ws && ws.readyState === WebSocket.OPEN) {
      // Ping senden (LogpyClaw ignoriert unbekannte Messages)
...
function connect() {
  ...
  try {
    ...
    console.error('[LogpyClaw] WS Konstruktor-Fehler:', e);
    scheduleReconnect();
    return;
  }

  ws.onopen = () => {
    console.log('[LogpyClaw] Verbunden mit', WS_URL);
    reconnectAttempts = 0;
    setState('connected');
    startKeepalive();
  };
  ...
  ws.onmessage = async (event) => {
    ...
    try {
      ...
      console.error('[LogpyClaw] Ungültiges JSON:', event.data?.slice(0, 100));
      return;
    }

    const { request_id, command, ...params } = msg;
    console.log('[LogpyClaw] Command:', command, 'id:', request_id?.slice(0, 8));
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      console.warn('[LogpyClaw] WS nicht offen — Antwort verloren:', request_id?.slice(0, 8));
    }
  };

  ws.onerror = (err) => {
    console.error('[LogpyClaw] WS Fehler:', err.type);
  };

  ws.onclose = (event) => {
    console.log('[LogpyClaw] WS geschlossen:', event.code, event.reason);
...
function scheduleReconnect() {
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    console.warn('[LogpyClaw] Max Reconnect-Versuche erreicht');
    return;
  }
  const delay = RECONNECT_BASE_MS * Math.min(Math.pow(1.5, reconnectAttempts), 15);
  reconnectAttempts++;
  console.log(`[LogpyClaw] Reconnect in ${Math.round(delay / 1000)}s (Versuch ${reconnectAttempts})`);
...
async function handleCommand(command, params) {
  try {
    ...
    console.error('[LogpyClaw] Command-Fehler:', command, e);
