...
den Cooldown. Ergebnisse inkrementell in experiments/dragon3-results.json.
"""
...
RESULTS = HERE / "dragon3-results.json"
...
def measure_rate(agent_id: str, n: int = 3) -> float:
    ...
    chat(agent_id, "Ein kurzer Satz: Du prüfst deine Ausrüstung.")
    times = []
    for _ in range(n):
        text, dt = chat(agent_id, "Ein kurzer Satz: Du prüfst deine Ausrüstung.")
        if not text.strip():
